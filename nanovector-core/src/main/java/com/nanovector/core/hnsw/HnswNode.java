 ...
 * <p>
 * Each node stores its internal ID, the maximum layer it participates in,
 * and an adjacency list of neighbor internal IDs per layer ({@code int[][] neighbors}).
 * <p>
 * <b>Design note:</b> {@code HnswNode} is a pure graph structure and has no dependency
 * on {@link com.nanovector.core.storage.VectorStorage} or distance calculations.
 * Vector data is accessed externally through {@code VectorStorage} by the orchestrating
 * {@code HnswIndex}.
 */
public final class HnswNode {
    ...
    /**
     * Adds a neighbor to this node's adjacency list at the specified layer.
    ...
    private void checkLayerBounds(int layer) {
        if (layer < 0 || layer > maxLevel) {
            throw new IndexOutOfBoundsException(
                    "Layer " + layer + " out of bounds for node " + internalId + " (maxLevel=" + maxLevel + ")");
        }
    }
}
