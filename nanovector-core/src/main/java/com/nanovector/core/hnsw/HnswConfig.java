 ...
 * <p>
 * Hyperparameters adhere to the paper by Malkov &amp; Yashunin (2018):
 * <ul>
 *   <li>{@code m}: Maximum number of outgoing edges per node at layers &gt; 0 (default: 16).</li>
 *   <li>{@code m0}: Maximum number of outgoing edges per node at layer 0 (default: 2 * m = 32).</li>
 *   <li>{@code efConstruction}: Size of dynamic candidate list during graph construction (default: 100).</li>
 *   <li>{@code efSearch}: Size of dynamic candidate list during nearest-neighbor search (default: 50).</li>
 *   <li>{@code mL}: Normalization factor for exponential level generation, typically {@code 1.0 / ln(m)}.</li>
 *   <li>{@code seed}: Optional random seed for deterministic level generation and testing (null for non-deterministic).</li>
 * </ul>
 */
public record HnswConfig(
    ...
    public HnswConfig {
        ...
        if (efConstruction <= 0) {
            throw new IllegalArgumentException("efConstruction must be positive, but got: " + efConstruction);
        }
        if (efSearch <= 0) {
    ...
    /**
     * Creates a default configuration with M = 16, M0 = 32, efConstruction = 100, efSearch = 50,
     * mL = 1 / ln(16), and non-deterministic random generation.
     */
