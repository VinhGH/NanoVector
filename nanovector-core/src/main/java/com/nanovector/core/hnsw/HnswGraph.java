...
/**
 * Multi-layer HNSW graph managing nodes, entry point, edge connections, pruning, and graph
 * traversal (greedy routing &amp; searchLayer).
 *
 * <p>Responsibilities:
 *
 * <ul>
 *   <li>Maintains the global {@code entryPoint} at the current maximum level ({@code maxLevel}).
 *   <li>Stores all {@link HnswNode}s indexed by their internal ID.
 *   <li>Provides bidirectional {@code connect} and {@code prune} operations that preserve the Graph
 *       Invariants (degree constraints, symmetric edges).
 *   <li>Implements {@code searchLayer} (Algorithm 2) and {@code greedyClosest} for multi-layer
 *       greedy routing.
 * </ul>
 *
 * <p><b>Design note:</b> {@code HnswGraph} does not hold vector data or compute distances. Distance
 * computations are injected via functional interfaces ({@link DistanceToQuery}, {@link
 * NeighborSelector.NodeDistanceEvaluator}) by the orchestrating {@code HnswIndex}.
 */
public final class HnswGraph {
   ...
   *
   * <p>The orchestrating {@code HnswIndex} creates this by closing over the query vector, {@code
   * VectorStorage}, and {@code DistanceCalculator}, keeping {@code HnswGraph} fully decoupled from
   * vector data.
   */
  @FunctionalInterface
  ...
  /**
   * Adds a new node to the graph. If this is the first node added, it becomes the initial entry
   * point. Subsequent entry point updates are managed via {@link #setEntryPoint(int, int)}.
   *
   ...
   *
   * <p>Uses a Min-Heap (candidate queue) to explore the closest unvisited candidate, and a Max-Heap
   * (dynamic result set) bounded to {@code ef} to track the best results.
   *
   ...
   * @param visitedSet epoch-based visited tracking (caller must call {@code nextEpoch()}
   *     beforehand)
  ...
  /**
   * Greedy 1-nearest-neighbor descent within a single layer. Used during the upper-layer routing
   * phase (layers {@code Lmax} down to {@code targetLevel + 1}) where only the single closest node
   * is tracked.
   *
   * @param distanceToQuery distance function from query to any node
   ...
   */
  public int greedyClosest(DistanceToQuery distanceToQuery, int currentBestId, int layer) {
  ...
  /**
   * Creates a bidirectional edge between two nodes at the specified layer. If either node's degree
   * exceeds the maximum allowed, calls {@code prune} on that node.
   *
   ...
   */
  public void prune(
      HnswNode node, int layer, int maxDegree, NeighborSelector.NodeDistanceEvaluator evaluator) {
  ...
  /**
   * Returns the maximum degree for the given layer. Layer 0 uses {@code M0} (= 2M), layers &gt; 0
   * use {@code M}.
   */
