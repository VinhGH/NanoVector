 ...
 *
 * <p>Instead of allocating a new set or clearing a bitset for every query, this structure
 * increments an integer epoch counter ({@code currentEpoch++}). A node is considered visited if its
 * entry in {@code visitedEpoch[internalId] == currentEpoch}.
 *
 * <p><b>Performance guarantee:</b> Guarantees zero heap allocation and zero clearing overhead
 * during normal search operations once capacity is sufficient to hold all internal node IDs.
 *
 * <p>Safely handles integer overflow of the epoch counter by resetting the array when {@code
 * currentEpoch} reaches {@link Integer#MAX_VALUE}.
 */
public final class EpochVisitedSet {
  ...
  public EpochVisitedSet(int initialCapacity) {
    if (initialCapacity <= 0) {
      throw new IllegalArgumentException(
          "Initial capacity must be positive, but got: " + initialCapacity);
    }
  ...
  /**
   * Marks the given internal node ID as visited in the current epoch. Automatically expands
   * capacity if needed.
   *
   * @param internalId internal index of the node
   */
   ...
   *
   * <p>If the epoch counter reaches {@link Integer#MAX_VALUE}, the array is reset to prevent
   * integer overflow.
   */
