...
/**
 * Neighbor selection heuristic based on Algorithm 4 from Malkov &amp; Yashunin (2018), with a
 * deterministic fallback to prevent unnecessarily sparse local neighborhoods.
 *
 * <p>Standard nearest-neighbor selection (picking the closest $M$ candidates) often causes
 * clustered edges in the same direction, reducing angular diversity. Algorithm 4 ensures diverse
 * edge directions by selecting a candidate only if it is closer to the target than to any neighbor
 * already selected.
 *
 * <p><b>NanoVector Fallback:</b> If the strict heuristic selects fewer than {@code maxDegree}
 * neighbors and unselected candidates remain, the nearest remaining candidates are added up to
 * {@code maxDegree}.
 */
