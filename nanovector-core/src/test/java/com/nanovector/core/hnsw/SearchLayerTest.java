...
class SearchLayerTest {
     ...
     *
     * Layout (layer 0 only):
     *   Node 0 (pos=0) -- Node 1 (pos=10) -- Node 2 (pos=20) -- Node 3 (pos=30) -- Node 4 (pos=40)
     *
     * Each node is connected to its immediate neighbors forming a chain.
     */
    private static final float[] POSITIONS = {0f, 10f, 20f, 30f, 40f};

    private static final HnswConfig CONFIG = new HnswConfig(
            4, 8, 100, 50, 1.0 / Math.log(4), 42L
    );
    ...
    @Test
    @DisplayName("searchLayer: Finds closest nodes in a chain graph starting from a distant entry point")
    void testSearchLayerFindsClosestNodes() {
        ...
        visited.nextEpoch();
        List<NeighborSelector.Candidate> results = graph.searchLayer(
                queryAt(35f),
    ...
    @Test
    @DisplayName("Multi-layer routing: greedyClosest descends through upper layers to find entry for layer 0")
    void testMultiLayerGreedyRouting() {
        ...
        HnswGraph.DistanceToQuery queryDist =
                (id) -> Math.abs(positions[id] - 35f);
